...
export const dispatchRunTools = async (props: DispatchToolModuleProps): Promise<Response> => {
  ...
  try {
    ...
    const shouldAdaptFilePrompt = hasReadFilesTool && !autoInjectFileContent;
          ...
          if (item.obj === ChatRoleEnum.Human) {
            return {
              ...
              value: toolCallMessagesAdapt({
                ...
                skip: !shouldAdaptFilePrompt, // 自动注入或无读文件工具时跳过文件元数据提示
...
const getMultiInput = async ({
  ...
  // No file quote, readFiles node will handle files, or auto inject disabled
  if (!fileLinks) {
    return {
      documentQuoteText: '',
  ...
  if (hasReadFilesTool || !autoInjectFileContent) {
    return {
      documentQuoteText: '',
